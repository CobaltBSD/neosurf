 ...
 */
#define NSGTK_WEBSEARCH_WIDTH 350
			      ...
			      gpointer data)
{
	...
	if (itemid == HISTORY_BUTTON) {
		...
		alloc->width = 20;
	} else if (tb->items[itemid].location <= tb->items[WEBSEARCH_ITEM].location) {
		alloc->x -= tb->offset;
		if (itemid == WEBSEARCH_ITEM) {
...
openlocation_button_clicked_cb(GtkWidget *widget, gpointer data)
{
	...
	urltitem = tb->items[WEBSEARCH_ITEM].button;
	if (urltitem != NULL) {
		GtkEntry *entry;
...
nserror nsgtk_toolbar_page_info_change(struct nsgtk_toolbar *tb)
{
	...
	url_entry = GTK_ENTRY(gtk_bin_get_child(GTK_BIN(tb->items[WEBSEARCH_ITEM].button)));
					 ...
					 struct nsgtk_pi_window *win)
{
	...
	res = nsgtk_toolbar_get_icon_window_position(tb, WEBSEARCH_ITEM, &x, &y);
	if (res != NSERROR_OK) {
		return res;
	}
